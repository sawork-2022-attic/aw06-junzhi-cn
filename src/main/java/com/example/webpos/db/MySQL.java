...
public class MySQL implements PosDB {
    ...
    private List<Product> parseMySQL() throws Exception {
        ...
        Connection connection = DriverManager.getConnection(
                "jdbc:mysql://localhost:3306/amazon?serverTimezone=UTC&useUnicode=true&characterEncoding=UTF-8", "root",
                "123456");
        ...
        while (resultSet.next()) {
            ...
            showList.add(new Product(asin, title, random.nextInt(300), images.get(random.nextInt(images.size())),
                    main_cat, categories, images));
        }
        resultSet.close();
        stmt.close();
        connection.close();
